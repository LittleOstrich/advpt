...
class Stencil : public MatrixLike<T, Stencil<T, rows, cols>, rows,
                                  cols> /* TODO: inherit MatrixLike */ {
public:
  Stencil(const std::vector<StencilEntry<T>> &boundaryEntries,
          const std::vector<StencilEntry<T>> &innerEntries)
      : boundaryStencil_(boundaryEntries), innerStencil_(innerEntries) {}
  Stencil(const std::vector<StencilEntry<T>> &
              innerEntries) // c'tor for stencils w/o explicit boundary handling
  ...
  // HINT: stencil entries are stored as offset/coefficient pair, that is the
  // offset specifies which element of a vector, relative to the current index,
  // is to be regarded. It is then multiplied with the according coefficient.
  // All of these expressions are evaluated and then summed up to get the final
  // result.
...
protected:
  ...
  // and last rows of a corresponding matrix and are to be applied to the first
  // and last element of a target vector; inner stencils correspond to the
  // remaining rows of the matrix
  std::vector<StencilEntry<T>>
      boundaryStencil_; // feel free to change the datatype if convenient
  std::vector<StencilEntry<T>>
      innerStencil_; // feel free to change the datatype if convenient
...
const T &Stencil<T, rows, cols>::operator()(int row, int col) const {
  if (row == 0 && col == 0)
    return boundaryStencil_[0].second;
  else if (row == size_ - 1 && col == size_ - 1)
    return boundaryStencil_[0].second;
  else if (row == col)
    return innerStencil_[1].second;
  else if (row == col + 1)
    return innerStencil_[0].second;
  else if (row == col - 1)
    return innerStencil_[2].second;
  else
    return rest_;
}
...
operator*(const Vector<T, rows> &rhs) const {
  ...
  auto inner1 =
      std::find_if(innerStencil_.begin(), innerStencil_.end(),
                   [](const StencilEntry<T> &p) { return p.first == 0; });
  auto inner2 =
      std::find_if(innerStencil_.begin(), innerStencil_.end(),
                   [](const StencilEntry<T> &p) { return p.first == 1; });
...
Stencil<T, rows, cols> Stencil<T, rows, cols>::inverseDiagonal() const {
  auto inner =
      std::find_if(innerStencil_.begin(), innerStencil_.end(),
                   [](const StencilEntry<T> &p) { return p.first == 0; });
  Stencil<T, rows, cols> tmp({{0, 1.}},
                             {{-1, 0}, {0, 1.0 / inner->second}, {1, 0}});
  tmp.setSize(size_);
  return tmp;
}

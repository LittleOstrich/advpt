...
using std::size_t;
template <size_t rows, size_t cols> using MatrixD = Matrix<double, rows, cols>;
...
bool almostEqual(const Matrix<double, a_rows, a_cols> &a,
                 const Matrix<double, b_rows, b_cols> &b,
                 double epsilon = 1e-13) {
  static_assert(a_rows == b_rows, "dimension mismatch in comparison");
  // TODO static_assert(a_cols == b_cols, "dimension mismatch in comparison");
...
void test_arithmetic() {
  ...
  constexpr size_t cols = 4;
  using MatrixDRC = MatrixD<rows, cols>;
  std::function<double(size_t, size_t)> f = [](size_t i, size_t j) {
    return j * 100 + i;
  };
  MatrixDRC m1(0);
  MatrixDRC m2(0);
  ...
  assert("check operator*=" &&
         almostEqual(prodn1n1, MatrixD<n1, n1>(n1 * a * b)));
  ...
  // TODO:: auto result = unity * v;
  for (size_t i = 0; i < n1; ++i) {
    // TODO    assert("check matrix * vector" && almostEqual(v(i),
    // result(i)));
  }
}
...
/*
    ...
    // n1 may be greater than n2, since underflow/overflow is well defined for
    // unsigned integer arithmetic (wrap around)
  ...
  static_assert(diff == diff_reference,
                "Matrix class must store its data on the stack");
}
...
void static_stack_usage_check_vector() {
  ...
  static_assert(diff == diff_reference,
                "Vector class must store its data on the stack");
}
...
void matrix_test2() {
  Matrix<double, 3, 3> m1(2.2);
  Matrix<double, 3, 3> m2(1.2);
  Matrix<double, 3, 3> m3;

  //  m3 = m1 + m2;
  //  for (size_t i = 0; i < m3.rows() * m3.cols(); i++) {
  //    //  std::cout << m3.data[i] << std::endl;
  //  }
  m3 = m1 - m2;
  for (size_t i = 0; i < m3.rows() * m3.cols(); i++) {
    std::cout << "i=" << i << " " << m3.data[i] << std::endl;
  }
  //  m3 = m1 * m2;
  //  for (size_t i = 0; i < m3.rows() * m3.cols(); i++) {
  //    //   std::cout << m3.data[i] << std::endl;
...
void matrix_test3() {
  Matrix<double, 10, 4> m1(1.2);
  Matrix<double, 4, 3> m2(1.2);
...
void matrix_test4() {
  Matrix<double, 1, 1> m1(1);
  Matrix<double, 2, 2> m2;
}

void matrix_test5() {
  Matrix<double, 2, 2> m1(1);
  Matrix<double, 2, 2> m2(1);
  Matrix<double, 2, 2> m3(m1);
  Matrix<double, 2, 3> m4(1);
  Matrix<double, 2, 2> m5(2);
  ...
  std::cout << "Test5 was passed" << std::endl;
}

...
template <size_t numPoints> constexpr double hxCalc() {
  return 1. / (numPoints - 1);
}
template <size_t numPoints> constexpr double hxSqCalc() {
  return hxCalc<numPoints>() * hxCalc<numPoints>();
}
...
template <typename container_t> container_t shuffled(container_t container) {
...
int solve(const MatrixLike<T, MatrixImpl, numPoints, numPoints> &A,
  ...
  while (
      curRes >
      1.e-5 *
          initRes) { // solve until the residual is reduced by a certain amount
    ++curIt;
...
std::pair<int, double> testStencil(const Vector<double, numPoints> b) {
  ...
  Vector<double, numPoints> u(0.);
  std::vector<StencilEntry<double>> innerStencil{
...
template <size_t numPoints> void testImpl(int expectedNumIts) {
  std::cout << "Checking " << numPoints << " grid points and expecting "
            << expectedNumIts << " iterations:" << std::endl;

  Vector<double, numPoints> b(0.0);
  for (size_t x = 0; x < numPoints; ++x) {
    b(x) = sin(2. * PI * (1. + x / (double)(numPoints - 1))) +
           cos(PI * (x / (double)(numPoints - 1)));
  }

  auto resMatrix = testFullMatrix<numPoints>(b);
  auto resStencil = testStencil<numPoints>(b);

  std::cout << "\tThe matrix implementation required  " << resMatrix.first
            << " iterations and " << resMatrix.second << " seconds"
            << std::endl;
  std::cout << "\tThe stencil implementation required " << resStencil.first
            << " iterations and " << resStencil.second << " seconds"
            << std::endl;
  std::cout << "\tThis means a speedup factor of "
            << resMatrix.second / resStencil.second << std::endl;

  assert(resMatrix.first == resStencil.first &&
         "Number of iterations not equivalent for matrix-stencil comparison");
  assert(resStencil.first == expectedNumIts &&
         "Number of iterations required does not match expected result");
  assert(resMatrix.second > resStencil.second &&
         "Runtime of the stencil test case is too high");
}
...
template <size_t curTestCase> void runTestsImpl(std::list<int> &expected) {
  testImpl<curTestCase>(expected.front());
}

template <size_t... testcases> void runTests(std::list<int> &expected) {
  runTestsImpl<testcases...>(expected);
}

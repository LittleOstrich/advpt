...
#include "Matrix.h"
#include "Vector.h" // empty, but that's ok (for reference check Assignment)
#include "Stencil.h"
#include <functional>
...
void solve(const MatrixLike<T, Derived, numPoints, numPoints> &A,
    ...
    while (curRes >1.e-5 *initRes) {
        // solve until the residual is reduced by a certain amount
        ++curIt;
        ...
        if (0 == curIt % 500) // print some info every few steps
            std::cout << "Residual after iteration " << curIt << ":\t" << curRes
                      << std::endl;
    }

    std::cout << "Residual after iteration " << curIt << ":\t" << curRes
              << std::endl
              << std::endl; // print the final number of iterations and the final
    // residual
}

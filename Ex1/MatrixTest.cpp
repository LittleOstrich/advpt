...
void test_memory(size_t rows = 2, size_t cols = 4, double initValue = 1.0) {
  ...
  // copy ctor
  {
    ...
    assert("testing copy/move constructor" &&
           almostEqual(m1, move_constructed, 0.0));
  }
...
// void test_input_output_self_consistency(size_t row = 2, size_t col = 4) {
...
// operator<< 	assert("check output and input operator" && almostEqual(m1, m2,
// 1e-4));
//}
